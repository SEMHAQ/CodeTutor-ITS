...
\midrule
BLEU Score & 0.119 & 0.064 & 0.062 & 0.494 \\
ROUGE-L Score & 0.129 & 0.051 & 0.061 & 0.278 \\
Response Time (s) & 29.19 & 12.10 & 5.91 & 47.83 \\
Word Count & 381.8 & --- & 42 & 312 \\
Has Code Block (\%) & 74.0 & --- & --- & --- \\
Has Example (\%) & 74.0 & --- & --- & --- \\
\bottomrule
\end{tabularx}
...
\begin{table}[H]
\caption{Prompt strategy comparison results.\label{tab:prompt}}
...
\toprule
\textbf{Configuration} & \textbf{Time (s)} & \textbf{Success} & \textbf{Length (chars)} \\
\midrule
Full System & 16.72 & 1.00 & 1487.3 \\
w/o Knowledge Tracking & 14.69 & 1.00 & 1264.5 \\
w/o Adaptive Hints & 17.69 & 1.00 & 1581.7 \\
Basic Tutor & 15.55 & 1.00 & 1336.1 \\
\bottomrule
\end{tabularx}
\end{table}

All configurations achieve 100\% success rate. Removing knowledge tracking reduces response length by 15.0\%, indicating its role in generating more focused, context-aware responses.
...
The experiments yield several important findings. First, the system achieves a mean BLEU score of 0.119 and ROUGE-L of 0.129, which reflect the nature of educational responses---the system generates comprehensive explanations that go beyond concise reference answers. Second, chain-of-thought prompting significantly outperforms zero-shot and few-shot approaches in completeness (0.72 vs. 0.24--0.32). Third, knowledge tracking contributes 15\% to response comprehensiveness. Fourth, average response times range from 14.7s to 29.2s, which are acceptable for educational applications. Fifth, the LLM-as-Judge evaluation confirms high response quality (4.51/5.0), with particularly strong performance in accuracy (4.70) and educational value (4.64), validating the system's pedagogical effectiveness.

\subsection{Educational Implications}

The results have several implications for programming education: (1) open-source LLMs can provide effective tutoring without ongoing costs; (2) local deployment ensures student data privacy; (3) the adaptive hint mechanism effectively supports struggling students; and (4) the modular architecture allows customization for specific curricula.
