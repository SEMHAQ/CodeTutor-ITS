...
Several studies have specifically explored LLMs for programming education. Sarsa et al.~\cite{ref18} used GPT-3 to generate programming exercises and solutions. Kiesler et al.~\cite{ref19} evaluated ChatGPT's ability to provide feedback on student code. Leinonen et al.~\cite{ref20} investigated using LLMs to explain programming errors to novices. LLM-based coding assistants such as GitHub Copilot~\cite{ref21} and Amazon CodeWhisperer~\cite{ref22} have demonstrated the potential of LLMs for code generation and completion, though they are designed as productivity tools rather than educational systems---they provide direct code rather than guided instruction. Prompt optimization techniques such as AutoHint~\cite{ref23} have shown that automated prompt engineering can improve LLM responses for educational tasks, but these approaches focus on prompt quality rather than integrated tutoring system design. These tools highlight the demand for AI-assisted programming support while underscoring the need for pedagogically informed systems that promote learning over dependency.
...
CodeTutor-ITS is grounded in three established pedagogical theories. First, Vygotsky's Zone of Proximal Development (ZPD)~\cite{ref27} posits that learning is most effective when instruction targets the gap between what a learner can do independently and what they can achieve with guidance. Our adaptive hint system operationalizes this by providing graduated support---from directional hints to complete solutions---that adapts to the student's current competence level. Second, scaffolded instruction~\cite{ref28} argues that temporary support structures should be progressively removed as learners gain competence. The four-level hint system implements this principle: students receive increasingly specific hints only after demonstrating persistent difficulty, encouraging independent problem-solving before resorting to direct answers. Third, the worked example effect~\cite{ref29} demonstrates that studying worked solutions is more effective for novice learners than problem-solving alone. The dialogue tutoring module incorporates this by providing structured code examples with detailed comments, particularly for students encountering new concepts. These theoretical foundations distinguish CodeTutor-ITS from generic LLM chatbots, which lack pedagogical structure and tend to provide direct answers rather than guided instruction.
