...
We compared three prompting strategies using 15 programming questions (5 per strategy, one per knowledge domain). While the per-strategy sample is small ($n = 5$), the questions span five distinct knowledge domains (Python basics, data structures, algorithms, OOP, and web basics), providing coverage across the curriculum. Table~\ref{tab:prompt} presents the results.

\begin{table}[H]
\caption{Prompt strategy comparison results ($n = 5$ per strategy, one representative question per knowledge domain). Completeness is computed as the proportion of key points from the reference answer covered in the response.\label{tab:prompt}}
\begin{tabularx}{\textwidth}{CCCCCC}
\toprule
