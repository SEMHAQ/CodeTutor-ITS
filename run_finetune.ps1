...
# Step 1: Generate training data (skip if exists)
if (Test-Path "experiments/data/training_data.jsonl") {
    $lines = (Get-Content "experiments/data/training_data.jsonl" | Measure-Object -Line).Lines
    Write-Host "[1/5] Training data already exists ($lines examples), skipping generation."
} else {
...
if (Test-Path "experiments/models/qwen2.5-7b-lora/final") {
    Write-Host "[3/5] Fine-tuned model already exists, skipping training."
} else {
